...
def QC_file_view(request,id):
    ...
    try:   
        ...
        from_date=sorted_date[0]
        to_date=sorted_date[-1] 
        data={'data':f1,'f':from_date,'t':to_date,'id':file_id,'c':check,'e':err,'uc':uncheck,'rj':reject}     
    except:
...
def Bulk_Data_save(request, id):
    ...
    if request.method == 'POST':
        ...
        messages.success(request,'Bulk Edited Successfull On Your File')
        return redirect(f'/qc_view_file/{id}')
    return redirect('/qc_home')
...
def Add_rows(request, id, pid):
    ...
    if request.method == 'POST':
        ...
        with open(f'{settings.BASE_DIR}//{file}', 'w') as f:
            json.dump(data1, f)
        messages.success(request,'Add Rows / Copy Records Successfully')
        return redirect(f'/qc_view_file/{id}')
    else:
        print('no')    
    data={'id':id,'pid':pid}
